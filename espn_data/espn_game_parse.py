...
def get_new_page(url, ua):
	return request.urlopen(request.Request(url, headers = { 'User-Agent' : ua.random }))

def get_page(url, ua):
	try:
		return get_new_page(url, ua)
	except SocketError as serr:
		if serr.errno == errno.ECONNRESET:
			print("Socket Error: %s failed. Wait two minutes for connection try reset" % (url))
			wait_time = 120
			time.sleep(wait_time)
			return get_new_page(url, ua)
		else: 
			try:
				wait_time = round(max(6, 5 + random.gauss(0,2.5)), 2)
				print("First attempt for %s failed. Wait %s seconds" % (url, wait_time))
				time.sleep(wait_time)
				return get_new_page(url, ua)
			except error.HTTPError as e:
				try:
					wait_time = round(max(60, 60 + random.gauss(0,2.5)), 2)
					print("Second attempt for %s failed. Wait %s seconds. Error: %s" % (url, wait_time, e.code))
					time.sleep(wait_time)
					return get_new_page(url, ua)
				except error.HTTPError as e:
					try:
						if e.code == 500:
							return 500
						wait_time = round(max(120, 120 + random.gauss(0,2.5)), 2)
						print("Third attempt for %s failed. Wait %s seconds. Error: %s" % (url, wait_time, e.code))
						time.sleep(wait_time)
						return get_new_page(url, ua)
					except error.HTTPError as e:
						if hasattr(e, 'reason'):
