...
if __name__ == '__main__':
    ...
    print_picks(today_resultsdf,check_pmargin=True,prob=.55)
=======
	...
	for idx, row in games.iterrows():
		if row['prob'] >= prob:
			...
			if float(row['results']) > 0:
				...
				spread = str(float(row['spread']) * -1)
				pmargin = str(row['pmargin'] * -1)
			bet = "Bet" if float(spread)+float(pmargin) >= 1 else "Stay away"
			print(winner.ljust(20),spread.ljust(5),pmargin.ljust(5),loser.ljust(20),str(round(row['prob'],2)).ljust(5),row['tipstring'].ljust(13),bet)
